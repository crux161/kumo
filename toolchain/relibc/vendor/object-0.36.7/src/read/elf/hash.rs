...
use super::{FileHeader, Sym, Version, VersionTable};
...
pub struct HashTable<'data, Elf: FileHeader> {
    pub buckets: &'data [U32<Elf::Endian>],
    pub chains: &'data [U32<Elf::Endian>],
}
...
impl<'data, Elf: FileHeader> HashTable<'data, Elf> {
    ...
    pub fn find<R: ReadRef<'data>>(
        ...
        while index != SymbolIndex(0) && i < self.chains.len() {
            if let Some(symbol) = symbols.get(index.0) {
...
pub struct GnuHashTable<'data, Elf: FileHeader> {
    pub symbol_base: u32,
    pub bloom_shift: u32,
    pub bloom_filters: &'data [u8],
    pub buckets: &'data [U32<Elf::Endian>],
    pub values: &'data [U32<Elf::Endian>],
}
...
impl<'data, Elf: FileHeader> GnuHashTable<'data, Elf> {
    ...
    pub fn find<R: ReadRef<'data>>(
        ...
        let symbols = symbols.get(index.0..)?;
        let values = self
            .values
